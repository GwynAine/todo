...
// Чтение с АЦП преобразователя (настроен на потенциометр)
int read_Adc(int x)
{
    ADCON0bits.CHS = x;
...
void main(void) 
{
    ...
    while(1)
    {
        ...
        if ((nl>500) && (nr>500))
        {
            motor_a_change_Speed(maxspeed);
            motor_b_change_Speed(maxspeed);
        }
        if ((nl>500) && (nr<500))
        {
            motor_a_change_Speed(maxspeed);
            motor_b_change_Speed(-25);
        }
        if ((nl<500) && (nr>500))
        {
            motor_a_change_Speed(-25);
            motor_b_change_Speed(maxspeed);
        }
        if ((nl<500) && (nr<500))
        {
